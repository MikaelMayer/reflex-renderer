...
Lexeme* Lexeur::readChain() {
	...
	if(count==1) {//Variables pr�d�finies
		...
		if(*debut == L'x')
			return new LexIdentity_x();
		if(*debut == L'y')
			return new LexIdentity_y();
	...
	return new LexFunction(debut, count);//Une cha�ne de caract�res est une fonction, pour l'instant.
...
Lexeme* Lexeur::readVariable() {
	...
	//Variables pr�d�finies � mettre ici.
...
Function *Parseur::lireX() {
  ...
  if (!E1)
	...
	if(E1->isOpeningParenthesis()) {
		...
		throw f;
	} else if(E1->isIdentity()) {
		avancerLex();
		return E1->getFunction1(NULL);
	} else if(E1->isFloat()) {
  ...
  bool function_valid = true;
	...
	try {
		...
		if(E1->isAlea()!=0) {
			...
			lireVirgule();
			F2 = lireE();
			Total = E1->getFunctionBinaire(Total,F2);
      F2 = NULL;
        ...
        F4 = new Constante(1.0);
			...
			Total = E1->getFunctionIncrementale(Total, v, F2, F3, F4);
      F2 = NULL;
      F3 = NULL;
      F4 = NULL;
		} else {
...
Variable* Parseur::lireVariable() {
  if(!lexCourant || !lexCourant->isVariable()) {
	...
	avancerLex();
	return VariableListe::get()->getVariable(E1->valueVariable());
}
