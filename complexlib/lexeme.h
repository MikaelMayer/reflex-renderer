...
public:
	...
	virtual Function *getFunction1(Function *argument1)	{throw PARSE_ERROR_SPECIALIZED;}
  virtual FunctionFactory* getFunctionFactory()	{throw PARSE_ERROR_SPECIALIZED;}
	virtual Function *getOperator(Function *argument1, Function *argument2)	{throw PARSE_ERROR_SPECIALIZED;}
	virtual Function *getOperatorPuissance(Function *argument1, int argument2)	{throw PARSE_ERROR_SPECIALIZED;}
	virtual ~Lexeme() {}
};

class LexIdentity:public Lexeme {public:
	LexIdentity() {}
	virtual bool isIdentity()	{return true;}
	virtual Function *getFunction1(Function *argument1) {return Identity::get();}
};

class LexIdentity_x:public LexIdentity {public:
	Function *getFunction1(Function *argument1) {return Identity_x::get();}
};

class LexIdentity_y:public LexIdentity {public:
	Function *getFunction1(Function *argument1) {return Identity_y::get();}
};
...
class LexFunction:public Lexeme {public:
	TCHAR *nom;
	LexFunction(const TCHAR* debut, int count) {
  ...
  Function *getFunction1(Function *argument1) {
		       if(_tcscmp(nom, TEXT("sin"))==0) {
			return new Sin(argument1);
		} else if(_tcscmp(nom, TEXT("cos"))==0) {
			return new Cos(argument1);
		} else if(_tcscmp(nom, TEXT("tan"))==0) {
			return new Tan(argument1);
		} else if(_tcscmp(nom, TEXT("exp"))==0) {
			return new Exp(argument1);
		} else if(_tcscmp(nom, TEXT("sinh"))==0) {
			return new Sinh(argument1);
		} else if(_tcscmp(nom, TEXT("cosh"))==0) {
			return new Cosh(argument1);
		} else if(_tcscmp(nom, TEXT("tanh"))==0) {
			return new Tanh(argument1);
		} else if(_tcscmp(nom, TEXT("ln"))==0) {
			return new Ln(argument1);
		} else if(_tcscmp(nom, TEXT("sqrt"))==0) {
			return new Sqrt(argument1);
		} else if(_tcscmp(nom,TEXT("argsh"))==0) {
			return new Argsh(argument1);
		} else if(_tcscmp(nom, TEXT("argch"))==0) {
			return new Argch(argument1);
		} else if(_tcscmp(nom, TEXT("argth"))==0) {
			return new Argth(argument1);
		} else if(_tcscmp(nom, TEXT("arcsin"))==0) {
			return new Arcsin(argument1);
		} else if(_tcscmp(nom, TEXT("arccos"))==0) {
			return new Arccos(argument1);
		} else if(_tcscmp(nom, TEXT("arctan"))==0) {
			return new Arctan(argument1);
		} else if(_tcscmp(nom, TEXT("real"))==0) {
			return new Real(argument1);
		} else if(_tcscmp(nom, TEXT("imag"))==0) {
			return new Imag(argument1);
		} else if(_tcscmp(nom, TEXT("circle"))==0) {
			return new Circle(argument1);
		} else if(_tcscmp(nom, TEXT("conj"))==0) {
			return new Conj(argument1);
		} else
			throw PARSE_ERROR_UNKNOWN_FUNCTION;
	}
	Function *getFunctionBinaire(Function *argument1, Function *argument2) {
		if(_tcscmp(nom, TEXT("o"))==0) {
			return new Compose(argument1, argument2);
		} else
			throw PARSE_ERROR_UNKNOWN_FUNCTION;
	}
	Function *getFunctionRecursive(Function *argument1, int argument2) {
		if(_tcscmp(nom, TEXT("oo"))==0) {
			return new CompositionRecursive(argument1, argument2);
		} else
			throw PARSE_ERROR_UNKNOWN_FUNCTION;
	}
	Function *getFunctionIncrementale(Function *argument, Variable* var, Function* debut, Function* fin, Function* step) {
		if(_tcscmp(nom, TEXT("sum"))==0) {
			return new SommeMultiple(argument, var, debut, fin, step);
		} else if(_tcscmp(nom, TEXT("prod"))==0) {
			return new ProduitMultiple(argument, var, debut, fin, step);
		} else if(_tcscmp(nom, TEXT("comp"))==0) {
      step->kill();
			return new CompositionMultiple(argument, var, debut, fin);
		} else
			throw PARSE_ERROR_UNKNOWN_FUNCTION;
	}
...
class LexOperator:public Lexeme {public:
	...
	Function *getOperator(Function *argument1, Function *argument2) {
		if(nom==L'+') {
			return new Somme(argument1, argument2);
		} else if(nom==L'*') {
	...
	Function *getOperatorPuissance(Function *argument1, int argument2) {
		if(nom==L'^') {
			return new Exposant(argument1, argument2);
		}
		throw PARSE_ERROR_UNKNOWN;
	}
};
...
class LexVariable:public Lexeme{public:
	LexVariable(const TCHAR* debut, int count) {
		nom = (TCHAR*)malloc((count+1)*sizeof(TCHAR));
		memcpy(nom, debut, count*sizeof(TCHAR));
		*(nom+count)=L'\0';	//Terminal character.
	}
